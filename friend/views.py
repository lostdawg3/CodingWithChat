	...
	if request.method == "POST" and user.is_authenticated:
		...
		if user_id:
			try:
				...
				# print("user for friendlist: " + str(user))
				# console.log(f"friend_list is " + {str(friend_list)} + "removee is " + {str(removee)} + "user_id is " + {str(user_id)} + "user is " + {str(user)})
				friend_list.unfriend(removee)
				# print("removee unfriended: " + str(removee))
				payload['response'] = "Successfully removed that friend."
			except Exception as e:
				payload['response'] = f"Something went wrong: {str(e)}"
				# payload['response'] = f"Something went wrong: {str(e)} + friend_list is + {str(friend_list)} + removee is  + {str(removee)} + user_id is  + {str(user_id)} + user is  + {str(user)}"
		else:
...
def decline_friend_request(request, *ags, **kwargs):
	user = request.user 
	payload = {}
	if request.method == "GET" and user.is_authenticated:
		friend_request_id = kwargs.get("friend_request_id")
