...
class NotificationConsumer(AsyncJsonWebsocketConsumer):
		...
		try:
			if command == "get_general_notifications":
				...
				if payload == None:
					...
					payload =json.loads(payload)
					await self.send_update_friend_request_notification(payload['notification'])
				...
				if payload == None:
					...
					print("payload after send is " + str(payload))
				...
				# else:
				# 	# send an error report for diagnostics not for production
	...
	async def send_update_friend_request_notification(self, notification):
		"""
...
def refresh_general_notifications(user, oldest_timestamp, newest_timestamp):
	...
	payload = {}
	if user.is_authenticated:
		oldest_ts = oldest_timestamp[0:oldest_timestamp.find("+")] # Remove the timestamp (we don't care)
		...
		print("refresh_general: oldest " + str(oldest_ts) + " newest " + str(newest_ts))
