...
class ChatConsumer(AsyncJsonWebsocketConsumer):
	...
	async def join_room(self, room_id):
		...
		# add them to the group so that they get messages
	...
	async def leave_room(self, room_id):
		...
		await self.channel.group_discard(
			room.group_name,
			self.channel_name
		)
...
def get_room_chat_messages(room, page_number):
	...
	try:
		...
		print("Number of messages: " + str(new_page_number))
...
def on_user_connected(room, user):
	...
	if user in connected_users:
		try:
			...
			UnreadChatRoomMessages(room=room, user=user, count=0).save()
			pass
	return
